...
namespace ASCOM.SnowFlakeProxy
{
    internal sealed class SnowflakeProxyController : IDisposable
    {
        ...
        internal SnowflakeProxyController(
            ...
            ProxyLogger logger)
            : this(create_underlying, settings, logger, ProxyIdentity.Slot1)
        {
        }
            ...
            ProxySlotIdentity identity)
        {
            ...
            this.hardware_worker = new HardwareWorker(create_underlying, this, settings, logger, identity);
        ...
        internal string DriverInfo
        {
            get
            {
                string info = "SnowFlakeProxy " + ProxyIdentity.DriverVersion + "; proxies " + identity.vendor_prog_id;
                lock (state_lock)
                {
        ...
        private void EnsureClientConnected(Guid client_id, string operation)
        {
            lock (state_lock)
            {
                if (!connection_leases.Contains(client_id) || connection_state != ConnectionState.Connected)
                {
                    throw new NotConnectedException(identity.proxy_name + " is not connected: " + operation);
                }
            }
        }
