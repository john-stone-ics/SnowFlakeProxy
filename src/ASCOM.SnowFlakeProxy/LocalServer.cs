//
// SnowFlakeProxy Local COM Server
//
...
namespace ASCOM.LocalServer
{
    public static class Server
    {
        ...
        private static string localServerAppId = "{F69F46A0-E336-4ED5-8E8D-E66065D2D3A3}"; // Our AppId
        ...
        static void Main(string[] args)
        {
            ...
            TL = new TraceLogger("", "SnowFlakeProxy.LocalServer")
            {
            ...
            Thread.CurrentThread.Name = "SnowFlakeProxy Local Server Thread";
        ...
        private static string GetAscomDeviceType(Type driverType)
        {
            string type_name = driverType.Name;
            if (type_name.StartsWith("FilterWheel", StringComparison.Ordinal))
            {
                return "FilterWheel";
            }
        ...
        private static void RegisterObjects()
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Error while registering the server:\n" + ex.ToString(), "SnowFlakeProxy", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return;
            }
            ...
            foreach (Type driverType in driverTypes)
            {
                ...
                try
                {
                    ...
                    string deviceType = GetAscomDeviceType(driverType); // FilterWheel2/3 must still register as FilterWheel
        ...
        private static void UnregisterObjects()
        {
            ...
            DeleteUnnumberedSlot1ComRegistration();
        }

        /// <summary>
        /// Slot 1 used to register as ASCOM.SnowFlakeProxy.FilterWheel (no 1). Copy settings
        /// onto the numbered ProgID, then drop the old Chooser/COM entries.
        /// </summary>
        private static void MigrateUnnumberedSlot1ProgId()
        {
            string old_id = ASCOM.SnowFlakeProxy.ProxyIdentity.unnumbered_legacy_prog_id;
            string new_id = ASCOM.SnowFlakeProxy.ProxyIdentity.proxy_prog_id_1;
            try
            {
                using (Profile profile = new Profile())
                {
                    profile.DeviceType = "FilterWheel";
                    if (profile.IsRegistered(old_id))
                    {
                        if (profile.IsRegistered(new_id))
                        {
                            CopyProfileValueIfEmpty(profile, old_id, new_id, "trace_enabled");
                            CopyProfileValueIfEmpty(profile, old_id, new_id, "normalize_filter_names");
                            CopyProfileValueIfEmpty(profile, old_id, new_id, "move_timeout_ms");
                            CopyProfileValueIfEmpty(profile, old_id, new_id, "position_retry_delay_ms");
                            CopyProfileValueIfEmpty(profile, old_id, new_id, "connect_timeout_ms");
                            CopyProfileValueIfEmpty(profile, old_id, new_id, "setter_accept_timeout_ms");
                        }

                        profile.Unregister(old_id);
                        TL.LogMessage("RegisterObjects", "Unregistered legacy ProgID " + old_id);
                    }
                }
            }
            catch (Exception ex)
            {
                TL.LogMessageCrLf("RegisterObjects", "Legacy ProgID profile migrate failed: " + ex);
            }

            DeleteUnnumberedSlot1ComRegistration();
        }

        private static void CopyProfileValueIfEmpty(Profile profile, string old_id, string new_id, string value_name)
        {
            string old_value = profile.GetValue(old_id, value_name, string.Empty, string.Empty);
            string new_value = profile.GetValue(new_id, value_name, string.Empty, string.Empty);
            if (old_value.Length > 0 && new_value.Length == 0)
            {
                profile.WriteValue(new_id, value_name, old_value);
            }
        }

        private static void DeleteUnnumberedSlot1ComRegistration()
        {
            string old_id = ASCOM.SnowFlakeProxy.ProxyIdentity.unnumbered_legacy_prog_id;
            try
            {
                Registry.ClassesRoot.DeleteSubKeyTree(old_id, false);
            }
            catch (Exception)
            {
            }
        ...
        private static void ElevateSelf(string argument)
        {
            ...
            {
                TL.LogMessage("IsAdministrator", $"The SnowFlakeProxy was not " + (argument == "/register" ? "registered" : "unregistered because you did not allow it."));
                MessageBox.Show("The SnowFlakeProxy was not " + (argument == "/register" ? "registered" : "unregistered") + " because you did not allow it.", "SnowFlakeProxy", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                TL.LogMessageCrLf("IsAdministrator", $"Exception: {ex}");
                MessageBox.Show(ex.ToString(), "SnowFlakeProxy", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
        ...
        private static bool RegisterClassFactories()
        {
            ...
            foreach (Type driverType in driverTypes)
            {
                ...
                if (!factory.RegisterClassObject())
                {
                    ...
                    MessageBox.Show("Failed to register class factory for " + driverType.Name, "SnowFlakeProxy", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return false;
                }
            ...
            if (args.Length > 0)
            {
                switch (args[0].ToLower())
                {
                    ...
                    default:
                        ...
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding", "SnowFlakeProxy", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
