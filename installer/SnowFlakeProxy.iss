#define MyAppName "Wanderer Snowflake Filter Wheel 1 (Proxy)"
...
begin
  Result := RegKeyExists(HKLM, 'SOFTWARE\ASCOM\FilterWheel Drivers\ASCOM.WandererSnowflakeFilterWheel1.FilterWheel');
end;
...
begin
  ...
  begin
    if MsgBox('The Wanderer Snowflake Filter Wheel 1 driver is not registered on this PC. The proxy cannot talk to the wheel without it. Continue anyway?', mbConfirmation, MB_YESNO) = IDNO then
      Result := False;
  end;
end;
