...
6. ConformU full FilterWheel run against `ASCOM.SnowFlakeProxy1.FilterWheel`. Save report under `docs\ConformU\`.
7. NINA only, SkyGuard only, then both. Choose **Wanderer Snowflake Filter Wheel N (Proxy)** for the same N as the Wanderer slot that owns the wheel. Do not choose the vendor driver and not JustAHub.
